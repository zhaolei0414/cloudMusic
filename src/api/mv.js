...
export const getMVAll = (options) => get('/mv/all', options)

/**
 * 说明 : 调用此接口 , 可获取最新 mv
 */
export const getMVFirst = (options) => get('/mv/first', options)

/**
 * 说明 : 调用此接口 , 可获取网易出品 mv
 */
export const getMVExclusive = (options) => get('/mv/exclusive/rcmd', options)
 ...
 */
export const getSimiMV = (id) => get('/simi/mv', { mvid: id })
/**
 * 说明 : 调用此接口 , 传入 mvid ( 在搜索音乐的时候传 type=1004 获得 ) , 可获取对应 MV 点赞转发评论数数据
 */
export const getMVDetailInfo = (id) => get('/mv/detail/info', { mvid: id })
